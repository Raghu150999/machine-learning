...
def neural_net():
    ...
    nn.add_layer(8)
    nn.add_layer(1, activation='sigmoid')
    nn.launch_training(epochs=1000, learning_rate=0.05, batch_size=64)

def neural_net_with_dropout():
    '''
    Neural network architecture
    '''
    # Accuracy: 0.925 F1-score 0.925
    nn = NeuralNetwork()
    nn.add_layer(16)
    nn.add_dropout(0.8)
    nn.add_layer(16)
    ...
    nn.add_dropout(0.8)
    nn.add_layer(1, activation='sigmoid')
    nn.launch_training(epochs=2000, learning_rate=0.2, batch_size=64)
    nn.launch_training(epochs=500, learning_rate=0.01, batch_size=64)

if __name__ == "__main__":
    # logistic_regression()
    neural_net()
    # neural_net_with_dropout()
